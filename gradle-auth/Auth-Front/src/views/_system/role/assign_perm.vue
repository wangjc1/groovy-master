      ...
      <el-col :span="12">
        <el-card class="box-card">
          ...
          <el-tree @check-change="handleUpdateMenuPermForRole" show-checkbox ref="menuPermTreeRef" :filter-node-method="filterNode"
                   :data="buttonPermissionTree" :props="treeProps" node-key="pval" default-expand-all :expand-on-click-node="false"  @check="handleCheck">
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       * 更新角色的菜单权限
       */
      handleUpdateMenuPermForRole: debounce(function(item,checked){
      ...
      /**
       * 当点击复选框时，当取消按钮选中时，不取消其父节点选中状态，否则会导致在不选中按钮时菜单也没选中的问题
       */
      handleCheck: debounce(function(data,node) {
        //当前复选框是否被选中
        let currentNodeChecked =  node.checkedKeys.filter(key=>key==data.pval);
        //每次取消选中按钮时，选中其父节点
        if(data.ptype==permType.BUTTON && currentNodeChecked.length==0){
           this.$refs.menuPermTreeRef.setChecked(data.parent,true);
        }
      })
