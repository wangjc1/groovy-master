...
import com.wjc.constant.Root;
import com.wjc.entity.sys.SysUser;
import com.wjc.entity.sys.SysUserRole;
import com.wjc.service.sys.SysRoleService;
import com.wjc.service.sys.SysUserRoleService;
import com.wjc.service.sys.SysUserService;
import com.wjc.util.PageUtils;
...
public class SysUserController {
    ...
    public Json updateUserRole(@RequestBody String body) {
        ...
        List<Long> rids = json.getJSONArray("rids").toJavaList(Long.class);
    ...
    public Json findUserRoles(@PathVariable Long uid){
        String oper = "find user roles";
        log.info("{}, uid: {}", oper, uid);
        if (Objects.isNull(uid)){
            return Json.fail(oper, "无法查询当前用户的角色值：参数为空（用户id）");
        }
        List<Long> rids = sysRoleService.getRoleIdsByUserId(uid);
