...
public class MyStringUtil extends StringUtils{
    ...
    public static StringBuilder replaceSpecialChar(String replaceContentSrc,
        ...
        if(MyStringUtil.empty(oldReplaceContent)){
            return builder;    
        }
        ...
        if(MyStringUtil.equals(beginChar, splitChar)) {
        ...
        if(MyStringUtil.equals(endChar, splitChar)) {
            oldReplaceContent = oldReplaceContent.substring(0,
                    oldReplaceContent.length() - splitChar.length());
            flagReplace = true;
        }
        // 把中间的去掉
        String[] splitStrs = MyStringUtil.split(oldReplaceContent, splitChar);
        if(flagReplace) {
	...
	* @return 第二个字符串追加到第一个字符串后的String
	...
	public static String createSystemDataPrimaryKey() {
		return MyStringUtil.replace(createUUID(), "-", "");
	}
	...
	public static String invertedCommaFilter(String source) {
		if (source == null)
			...
			return MyStringUtil.replace(source, "'", "\\'");
	}
	...
	public static String fillSpacing(String source, String values[], int level) {
		if (source.indexOf("{?}") != -1) {
			...
			source = MyStringUtil.replace(source1, "{?}", values[level]) + source.substring(source.indexOf("{?}") + 3, source.length());
			level++;
     ...
     * @return
     */
    public static List<String> str2List(String str, String c) {
     ...
     * @return
     */
    public static List<String> strArray2List(String[] strs) {
        List<String> list = new ArrayList<String>();
    ...
    public static String strArray2str(String[] strArray, String c) {
        String str = "";
    ...
    public static String[] str2strArray(String str, String c) {
        String strArray[] = null;

        if (StringUtils.isNotEmpty(str)) {
            strArray = str.split(c);
        }
    ...
    public static String joinQryArraysByList(List<String> ls) {
        ...
        int length = ls.size();
        for (int i = 0; i < length; i++) {
            sb.append("'").append(ls.get(i)).append("'");
            if (i == length - 1) {
                break;
            } else {
                sb.append(",");
    ...
    /**
     * @param jsonString String json1 =
     *                   "[{\"proid\":\"030030002\",\"procomment\":1},{\"proid\":\"040030001\",\"procomment\":1}]";
     * @param cls
     * @return
     */
    public static List getListByJsonStrAndClass(String jsonString, Class cls) {
        List list = new ArrayList();
        try {
            list = JSON.parseArray(jsonString, cls);// 阿里巴巴fastjson
        } catch (Exception e) {
        }
        return list;
    }
